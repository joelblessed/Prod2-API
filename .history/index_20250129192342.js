...
app.post("/signup", async (req, res) => {
  const { email, password, userName, fullName, phone, country, address, gender} = req.body;
  ...
  fs.readFile(accountPath, "utf8", async (err, data) => {
    ...
    const newUser = { id: db.users.length + 1, email, password: hashedPassword, userName, fullName, phone, country, address, gender};
...
app.post("/signin", async (req, res) => {
  ...
  fs.readFile(accountPath, "utf8", async (err, data) => {
    ...
    let db = JSON.parse(data);
    const user = db.users.find(user => user.email === email) || db.users.find(user => user.userName === userName);
