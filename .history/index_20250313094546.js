...
const productsRoutes = require("./jsFiles/db")
...
const formUploadRoutes = require("./jsFiles/formUploa");
const formUploadRoutes = require("./jsFiles/formUpload");
const formEditRoutes = require("./jsFiles/formEdit");
...
app.put("/cart/:id/increment", (req, res) => {
  fs.readFile(cartPath, "utf8", (err, data) => {
    ...
    // Find the item
    let item = db.carts.find(item => item.id === itemId);
...
//   const { email, password, userName, fullName, phone, country, address,city,wallet, gender,role} = req.body;
...
//     const newUser = { id: db.users.length + 1, email, password: hashedPassword, userName, fullName, phone, country, address,city,wallet, gender,role};
