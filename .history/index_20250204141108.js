...
app.post("/upload", upload.array("files",5), (req, res) => {
  if (!req.files|| req.file.length === 0) {return res.status(400).json({ message: "No file uploaded" });

  const { title, description } = req.body;
  const image = `${req.files.map(file =`upload${file.filename}`)}`;
  ...
  fs.readFile(dbPath, "utf8", (err, data) => {
    ...
    db.products.push({ id: db.products.length + 1, title, description, files:fileurls });

    fs.writeFile(dbPath, JSON.stringify(db, null, 2), (err) => {
      if (err) return res.status(500).json({ error: "Error saving post" });
      res.json({ message: "Post uploaded successfully", post: { title, description, files:fileurl } });
    });
