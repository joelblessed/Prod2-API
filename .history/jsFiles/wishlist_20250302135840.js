...
router.get("/wishlist", verifyToken, (req, res) => {
    ...
    const userWishList = db.wishlists.find(wishlist => wishlist.userId === req.userId)?.wishlist || [];
...
router.post("/wishlist", verifyToken, (req, res) => {
    ...
    let userWishList = db.wishlists.find(wishlist => wishlist.userId === req.userId)?.wishlist || [];
    ...
    if (userIndex !== -1) {
        db.wishlists[userIndex].wishlist = userWishList;
    } else {
        db.wishlists.push({ userId: req.userId, wishlist: userWishList });
    }

    writewishlistDB(db);
    res.json({ message: "Product added to wishlist", wishlist: userWishList });
});
...
router.post("/wishlist/merge", verifyToken, (req, res) => {
    ...
    const userIndex = db.wishlists.findIndex(wishlist => wishlist.userId === req.userId);
    let userWishList = userIndex !== -1 ? db.wishlists[userIndex].wishlist : [];

    // Merge wishlists and remove duplicates
    const mergedCart = [...new Map([...localCart, ...userWishList].map(item => [item.id, item])).values()];
