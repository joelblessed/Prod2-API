...
const getUsers = () => JSON.parse(fs.readFileSync("account.json", "utf8")).users;
const saveUsers = (users) => fs.writeFileSync("account.json", JSON.stringify({ users }, null, 2));

// Endpoint to apply reward to referrer when referred user makes a purchase
app.post("/apply-reward", (req, res) => {
