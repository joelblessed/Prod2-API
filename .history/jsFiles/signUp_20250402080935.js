...
const getUsers = () =>
  JSON.parse(fs.readFileSync("./jsonFiles/account.json", "utf8")).users;
const saveUsers = (users) =>
  fs.writeFileSync(
    "./jsonFiles/account.json",
  ...
  fs.readFile(accountPath, "utf8", async (err, data) => {
    ...
    fs.writeFile(accountPath, JSON.stringify(db, null, 2), (err) => {
      if (
        ...
        return res.status(500).json({ error: "Error saving database" });
      }
      res.json({
...
router.post("/apply-reward", (req, res) => {
  ...
  if (referredUser.referredBy) {
    ...
    if (referrer) {
      ...
      saveUsers(users);
      return res.json({
        message: `Reward applied! ${referrer.name} now has $${referrer.rewardBalance} reward balance.`,
      });
    }
  }
