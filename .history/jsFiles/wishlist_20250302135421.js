...
const readwishlistDB = () => {
    try {
        ...
        return { wishlist: [] }; // Default to empty if error occurs
    }
};
...
router.get("/wishlist", verifyToken, (req, res) => {
    ...
    const userCart = db.wishlist.find(cart => cart.userId === req.userId)?.cart || [];
...
router.post("/wishlist", verifyToken, (req, res) => {
    ...
    let userCart = db.wishlist.find(cart => cart.userId === req.userId)?.cart || [];
    ...
    // Find user index
    const userIndex = db.wishlist.findIndex(cart => cart.userId === req.userId);

    if (userIndex !== -1) {
        db.wishlist[userIndex].cart = userCart;
    } else {
        db.wishlist.push({ userId: req.userId, cart: userCart });
    }
...
router.delete("/wishlist/:productId", verifyToken, (req, res) => {
    ...
    const updatedCart = db.wishlist[userIndex].cart.filter(item => item.id !== parseInt(productId));
    db.wishlist[userIndex].cart = updatedCart;
    writewishlistDB(db);
...
router.post("/wishlist/merge", verifyToken, (req, res) => {
    ...
    const userIndex = db.wishlist.findIndex(cart => cart.userId === req.userId);
    let userCart = userIndex !== -1 ? db.wishlist[userIndex].cart : [];

    // Merge wishlist and remove duplicates
    const mergedCart = [...new Map([...localCart, ...userCart].map(item => [item.id, item])).values()];

    if (userIndex !== -1) {
        db.wishlist[userIndex].cart = mergedCart;
    } else {
        db.wishlist.push({ userId: req.userId, cart: mergedCart });
    }
...
router.delete("/wishlist", verifyToken, (req, res) => {
    let db = readwishlistDB();
    db.wishlist = db.wishlist.filter(cart => cart.userId !== req.userId);
    writewishlistDB(db);
