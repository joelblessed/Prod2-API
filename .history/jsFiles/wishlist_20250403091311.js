...
const ACCOUNT_PATH = path.join(__dirname, 'database/account.json');
...
app.get('/api/guest/wishlist', (req, res) => {
...
app.delete('/api/guest/wishlist/:productId', (req, res) => {
  const { productId } = req.params;
  const { deviceId } = req.body;
  const productIdNum = Number(productId);
...
// 4. Sync Endpoint
app.post('/api/wishlist/sync', authenticate, (req, res) => {
