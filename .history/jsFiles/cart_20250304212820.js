...
// 1. Get User Cart
router.get("/sercart", verifyToken, (req, res) => {
    const db = readCartDB();
...
// Increment Quantity
router.patch("/cart/:productId/increment", verifyToken, (req, res) => {
    ...
    const productIndex = db.carts[userIndex].cart.findIndex(item => item.id === parseInt(productId));
    if (productIndex === -1) return res.status(404).json({ message: "Product not found in cart" });

    // Increment the quantity of the product
    db.carts[userIndex].cart[productIndex].quantity += 1;

    // Save to database
    writeCartDB(db);

    res.json({ message: "Product quantity incremented", cart: db.carts[userIndex].cart });
});

// Decrement Quantity
router.patch("/cart/decrement/:productId", verifyToken, (req, res) => {
    const { productId } = req.params;
    ...
    // Decrement the quantity, and remove product if quantity reaches 0
    if (db.carts[userIndex].cart[productIndex].quantity > 1) {
        db.carts[userIndex].cart[productIndex].quantity -= 1;
    } else {
        db.carts[userIndex].cart.splice(productIndex, 1); // Remove product if quantity is 1
    }

    // Save to database
    writeCartDB(db);

    res.json({ message: "Product quantity decremented", cart: db.carts[userIndex].cart });
});
