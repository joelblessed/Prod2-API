...
router.post('/logs', (req, res) => {
  ...
  if (!userId || !productId || !action) {
    return res.status(400).json({ error: 'userId, action, and productId are required' });
  }
  ...
  // Make sure viewedProducts array exists
  if (!Array.isArray(latestLog.viewedProducts)) {
    latestLog.viewedProducts = [];
  }
  ...
  // Update the original db.logs by finding the exact log to update
  const logIndex = db.logs.findIndex(log => log.id === latestLog.id);
  if (logIndex !== -1) {
    db.logs[logIndex] = latestLog;
  }
