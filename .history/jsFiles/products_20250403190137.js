...
const readProductData = () => {
  try {
      ...
      return JSON.parse(data).products; // Assuming products are stored inside db.json
  } catch (error) {
      return [];
  }
};

// *Filter Products by Array of IDs*
router.post("/wishlist/filter", (req, res) => {
  const { productIds } = req.body;
  const products = readProductData();

  if (!Array.isArray(productIds) || productIds.length === 0) {
      return res.status(400).json({ message: "Invalid product ID array" });
  }

  // Filter products where product ID is in the given array
  const filteredProducts = products.filter((p) => productIds.includes(p.id.toString()));
